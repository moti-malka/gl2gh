...
class WebhookTransformer(BaseTransformer):
    ...
    def _transform_webhook(
        ...
        Returns:
            Transformed webhook data for GitHub, or None if webhook is invalid
        """
        ...
        # Note: insecure_ssl is stored as boolean here and converted to string ("0"/"1")
        # by CreateWebhookAction when creating the webhook via GitHub API
        transformed = {
            ...
            # Secret handling: GitLab API doesn't return secret values for security
            # Secrets must be regenerated for GitHub webhooks
            "secret": None  # Will require user input or auto-generation
        }
